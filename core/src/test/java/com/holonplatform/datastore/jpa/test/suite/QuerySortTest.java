...
public class QuerySortTest extends AbstractJpaDatastoreSuiteTest {
	...
	public void testSorts() {
		...
		res = getDatastore().query().target(JPA_TARGET).sort(KEY.asc()).list(KEY);
		assertEquals(2, res.size());
		assertEquals(Long.valueOf(1), res.get(0));

		res = getDatastore().query().target(JPA_TARGET).sort(QuerySort.asc(KEY)).list(KEY);
		assertEquals(2, res.size());
		assertEquals(Long.valueOf(1), res.get(0));

		res = getDatastore().query().target(JPA_TARGET).sort(QuerySort.of(KEY, SortDirection.DESCENDING)).list(KEY);
		assertEquals(2, res.size());
		assertEquals(Long.valueOf(2), res.get(0));
