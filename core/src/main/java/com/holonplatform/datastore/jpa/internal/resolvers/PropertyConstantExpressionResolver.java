...
public enum PropertyConstantExpressionResolver implements ExpressionResolver<PropertyConstantExpression, JPQLToken> {
	 ...
	 */
	@Override
	public Optional<JPQLToken> resolve(PropertyConstantExpression expression, ResolutionContext context)
		...
		return context.resolve(
				LiteralValue.create(expression.getModelValue(), expression.getModelType(),
						expression.getProperty().getConfiguration().getTemporalType()
								.orElse(JpaDatastoreUtils.getTemporalType(expression, true).orElse(null))),
