...
/**
 * Default {@link JPQLStatementResolutionContext} implementation.
 *
 * @since 5.1.0
 */
public class DefaultJPQLStatementResolutionContext extends DefaultJPQLResolutionContext
	 ...
	 */
	public DefaultJPQLStatementResolutionContext(JpaContext context, RelationalTarget<?> rootTarget,
			AliasMode aliasMode) {
		super(context);
		ObjectUtils.argumentNotNull(rootTarget, "Root target must be not null");
	 ...
	 * @param aliasMode Alias handling mode (not null)
	 */
	public DefaultJPQLStatementResolutionContext(JPQLResolutionContext parent, RelationalTarget<?> rootTarget,
			AliasMode aliasMode) {
		super(parent);
		...
		this.rootTarget = rootTarget;
		this.aliasMode = aliasMode;
		this.statementContextSequence = JPQLResolutionContext.getContextSequence(parent,
				JPQLStatementResolutionContext.class) + 1;
		init();
	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.holonplatform.datastore.jpa.context.JPQLStatementResolutionContext#getRootTarget()
	 */
	@Override
	public RelationalTarget<?> getRootTarget() {
		return rootTarget;
	}
	...
	public Optional<String> getAlias(Path<?> path, boolean useParentContext) {
		...
		if (!alias.isPresent() && useParentContext) {
			// check parent
			return getParent().filter(parent -> (parent instanceof JPQLStatementResolutionContext))
					.flatMap(parent -> ((JPQLStatementResolutionContext) parent).getAlias(path, true));
		}
		return alias;
	}
