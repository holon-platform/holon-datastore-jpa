...
Besides the standard link:holon-core.html#DatastoreExpressionResolverExtension[Datastore API expressions], such as `DataTarget`, `QueryFilter` and `QuerySort`, which can be used to extend the `Datastore` API with new expression implementations and new resolution strategies, the JDBC `Datastore` API can be extended using a set of *specific JPQL resolution expressions*, used by the internal _JPQL composer engine_ to implement the resolution and composition strategy to obtain JPQL statements from the `Datastore` API meta-language expressions.
